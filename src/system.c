/*
    Håndterer systemene. 
    !! Mange errors kommer fra report 'klassen'
    TODO: Luk ut unødvendig kode
*/
...
{   
  uint8_t char_counter = 1; // sjekker andre char i linjen
  uint8_t helper_var = 0; // Helper variable // ? 
  float parameter, value; // ? 
  switch( line[char_counter] ) { // switch case i forhold til hvilket tegn som har index 1 i linjen.
    case 0 : report_grbl_help(); break; // om det ikke er noe, send hjelp tekst til seriell monitor
    case '$': case 'G': case 'C': case 'X': // om $ G C X ->
      if ( line[(char_counter+1)] != 0 ) { return(STATUS_INVALID_STATEMENT); } // sjekk om neste er ulik 0, hvis ja da er det ugyldig
      switch( line[char_counter] ) { // switch case igjen for index 1
        case '$' : // 
